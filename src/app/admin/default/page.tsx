...
import { FaWind, FaChargingStation, FaPlug, FaInfoCircle } from 'react-icons/fa';
...
const chartData = [
  ...
  },
  {
    ...
    icon: <FaChargingStation className="text-4xl text-gray-800 dark:text-white" />,
    showChart: true,
  },
  {
    title: 'Connectors',
  ...
  },
  {
    ...
    topics: [
      ...
      { name: 'High Power', value: 44.10 },
      { name: 'Low Power ', value: 21.45 },
      { name: 'High Power', value: 54.10 },
      { name: 'Low Power ', value: 19.45 },
      { name: 'High Power', value: 65.10 },
    ],
...
const PieChartWidget = ({ title, data = [], labels = [], colors = [], icon, showChart, topics = [] }) => {
  const pieData = {
    labels: labels,
    datasets: [
      {
  ...
  return (
    <div className="rounded-lg bg-white shadow-lg dark:bg-navy-700 dark:border-navy-700 p-5 flex flex-col gap-3">
      {/* Info Section */}
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {showChart && <p className="text-4xl font-bold text-[#156082]">{total}</p>}
        </div>
      </div>
      ...
      {showChart ? (
        <div className="flex flex-col items-center">
          ...
          {/* Labels */}
          <div className="flex justify-between mt-3 w-full text-sm">
        ...
        <div className="mt-3">
          ...
          {topics?.map((topic, index) => (
            ...
            <div key={index} className="flex justify-between items-center mb-2">
              ...
              <span className="text-lg font-bold text-[#156082]">{topic.value}</span>
            </div>
          ))}
        </div>
      )}
